...
const RenderFilter = ({ setBase64Image }: Prop) => {
  ...
  const generateHaircut = async () => {
    if (radioRef.current) {
      ...
      const selectedOption = radioContainer.querySelector("input:checked") as HTMLInputElement;
      if (selectedOption) {
        ...
        if (fileimg) {
          ...
          try {
            setProcessing(true);
            const response = await fetch("https://flask-production-f2d6.up.railway.app/generate-haircut", {
              method: "POST",
              body: payload,
            });
...
const retryAgain = () => {
  ...
  <div style={{ marginTop: '10px' }} className="radio-buttons-container" ref={radioRef} >
